    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-100 p-6">
      ...
      <main className="max-w-5xl mx-auto space-y-10">
        ...
        <section className="space-y-4">
          {tasks.length === 0 ? (
            <p className="text-slate-500 italic text-center py-10">No tasks yet. Create one above!</p>
          ) : (
            sortedTasks.map((task) => {
              ...
              return (
                <div key={task.id} className={`relative p-5 border-2 rounded-2xl shadow-md hover:shadow-lg transition ${statusColors[task.status]}`}>
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    {task.status === 'ToDo' && <button onClick={() => updateTaskStatus(task.id, 'InProgress')} className="px-3 py-1.5 bg-yellow-500 hover:bg-yellow-600 rounded-md text-slate-900 font-medium text-sm">Start</button>}
                    {task.status === 'InProgress' && <button onClick={() => updateTaskStatus(task.id, 'Finished')} className="px-3 py-1.5 bg-green-600 hover:bg-green-700 rounded-md font-medium text-sm">Finish</button>}
                    <button onClick={() => deleteTask(task.id)} className="px-3 py-1.5 bg-red-600 hover:bg-red-700 rounded-md font-medium text-sm">Delete</button>
                  </div>
                </div>
              );
            })
